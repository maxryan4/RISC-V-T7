...
* [top.sv (single cycle)](#creating-topsv-single-cycle-file)
* [instruction tests](#creating-the-instruction-tests)
* [pipeline tests](#creating-pipeline-tests)
* [verification](#writing-the-verification-files)


## Creating [top.sv (single cycle)](../rtl/single-cycle/top.sv) file
...
I created the initial top.sv file, which succeeded in executing the [base program file](../tb/asm/).
To aid me with this task, I used the [single cycle CPU diagram](https://github.com/EIE2-IAC-Labs/Lab4-Reduced-RISC-V/blob/main/images/microarchitecture.jpg) that was provided in the lecture slides.
I made sure that all signal names matched the ones used in the diagram, or used in the modules outside of top.sv.
During my first tests of the program, I manually created a [program.hex]() file and filled it with the [program.S](../tb/asm/other/program.S) hex instructions.
Running this in the [test bench that was provided during lab 4](https://github.com/EIE2-IAC-Labs/Lab4-Reduced-RISC-V/tree/main/repo/tb) allowed me to confirm that the top file was working correctly.





## Creating the [instruction tests](../tb/asm/instruction_tests/)
...
The idea was to create a file which consisted of multiple tests. Each assembly file tested a specific category of instructions, for example, [arithmetictest.S](../tb/asm/instruction_tests/arithmetictest.S) tested simple arithmetic instructions such as sub and add. Most tests where tested by incrementing a0 for every test that was passed. For example, 4 instructions were being tested, I must check for a0 being equal to 4 in the testbench.
...
## Creating [pipeline tests](../tb/asm/pipeline_tests/)
...
This was the easiest assembly file to write and test, for [addtest_1.S](../tb/asm/pipeline_tests/addtest_1.S), I was checking if there would be an issue with performing arithmetic using a register that was altered in the previous clock cycle.

this, along with [addtest_2.S](../tb/asm/pipeline_tests/addtest_2.S) were the easiest to test, and worked as soon as the CPU had valid stalling.
...
## Writing the [verification files](../tb/tests/)
...
To test the CPU using the assembly files, I initially modified the [verified.cpp file](https://github.com/EIE2-IAC-Labs/Lab4-Reduced-RISC-V/blob/main/repo/tb/tests/verify.cpp) that was given to us in lab4. Later on, I then duplicated this file and had seperate files for single cycle and pipelined verification. This was important as the pipelined CPU went above and beyond the single cycle CPU. There were many instructions and programs that the pipelined CPU would support, but not supported within the single cycle CPU (such as [multest.S](../tb/asm/instruction_test)).

### Initial verification
Initially, the verification was done by running a program until a0 reaches the expected value, once this value was reached, the program would exit. This was especially good for programs such as [program.S](../tb/asm/other/program.S) where the program would restart if it wasn't terminated at the expected output.


### Separating of pipelined and single cycle CPU
Separating the pipelined and single cycle verification was key, as the pipelined required testing of a wider number of instructions and programs. It was also useful in debugging, for example if a program runs perfectly in the single cycle CPU, but does not run at all in the pipelined version, the issue is in the pipelined version. An example of a test that was only used in the pipelined version was [multest.S](../tb/asm/instruction_tests/multest.S), as mentioned above in the [instruction tests](#creating-the-instruction-tests) section.
