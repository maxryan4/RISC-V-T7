...
<<<<<<< HEAD
The ALU needed to be able to handle R, B and I instructions.
The R-type instructions are arithemtic instructions between two registers.
The I-type instructions are arithemtic instructions between the value in a register and an immediate.
The B-type instructions determine whether or not to branch based off of the relation between 2 registers such as if they are equal. If the condition that is being tested it branches to PC + offset.
...
The mul_sel signal is func7[0] which is is high if it is a multiplication or division instruction.
=======
...
<<<<<<< HEAD
The MULH, MULHSU, MULHU return the largest 16 bits based off of either a signed * signed, signed * unsigned or unsigned * unsigned interpretation of the operands.
